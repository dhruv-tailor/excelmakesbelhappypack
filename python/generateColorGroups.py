#https://gist.github.com/FaintSpeaker/69ede520d1859ade3f374eee7269f10b
#pip install colormath#pip install colormath
...
PARSER.add_argument("--output", metavar='Output Filename', default="output-file.txt", dest="output_filename", help="The filename to use for the output CB file. Defaults to output-file.txt")
PARSER.add_argument("--localisation", metavar='Output Filename', default="output-localisation.txt", dest="output_localisation", help="The filename to use for the output localisation file. Defaults to output-localisation.txt")
...
PARSER.add_argument("--blacklist", metavar="Tag Blacklist", default=["REB","PIR"], nargs="*", dest="blacklist_tags", help="A list of tags to blacklist from the generated CBs.")
...
localisation = open(ARGS.output_localisation, "w", encoding=ARGS.output_encoding)
...
for i, v in enumerate(list(GROUPS)):
	...
	localisation.write(""" cb_color_{0}_desc:0 "People have been complaining about unresolved margins, color disputes, precarious perimeters and a chroninc chromatic conflict between our nations. It can only be resolved on the battlefield."
